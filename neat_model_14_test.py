...
class TradingEnvironment:
    ...
    def step(self, action):
        ...
        if action == 1 and ema5_prev < ema8_prev and ema5_current > ema8_current and self.close_price[
            ...
            self.current_step] > ema100_current and self.close_price[self.current_step - 1] < ema100_prev and len(
            self.entry) == 0:
            loss_pct = 0.03
            gain_pct = 0.15
...
def test_model(network, test_data):
    ...
    print(f'Winrate: {round(winrate, 2) * 100}%, Wins: {win}, Loss: {loss}, PNL%: {round(PNL,2)}%')
    return action_list
