...
while True:
    try:
        ...
        class TradingEnvironment:
            ...
            def step(self, action):
                ...
                if action == 1 and ema5_prev < ema8_prev and ema5_current > ema8_current and self.close_price[
                    ...
                    self.current_step] > ema100_current and self.close_price[self.current_step - 1] < ema100_prev and len(self.entry)==0:
                    loss_pct = 0.03
                    gain_pct = 0.15
                ...
                elif action == 2 and len(self.entry) > 0:
        ...
        def run_neat(config_path, save_path="best_genome3.pkl", generations=300):
            """
            ...
            def evaluate_genomes(genomes, config):
                ...
                for genome_id, genome in genomes:
                    ...
                    env = TradingEnvironment(data_inputs)  # Assumes `data_inputs` is defined globally or passed in
                    env.reset()
                    ...
                    print(f'Count 1: {action_list.count(1)}')
                    print(f'Count -1: {action_list.count(-1)}')
                    print(f'Count 0: {action_list.count(0)}')
                    print(f'Count 2: {action_list.count(2)}')

                    genome.fitness = max(total_profit, 0)


                    # Print genome performance
                    print(f'Trader: {genome_id}, PNL%: {round(PNL, 2)}%')
